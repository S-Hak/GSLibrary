...
def download_tool(request,tool_id):
    ...
    if request.method == "GET":
        try:
            ...
            tool = Tool.objects.get(id=int(tool_id))
            #bug - 重复输入路径 - ubuntu21.04
            find_filename = BASE_DIR + tool.path.split('/zslibrary')[-1]
            if not os.path.isfile(find_filename):
                find_filename = tool.path.split('/zslibrary')[-1]
            
            response = StreamingHttpResponse(file_iterator(find_filename))
